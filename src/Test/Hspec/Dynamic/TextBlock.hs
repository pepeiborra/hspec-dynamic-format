...
module Test.Hspec.Dynamic.TextBlock
  ( TextSpan(..)
  , TextBlock(..)
  , textBlock
  , TextBlocks
...
instance Functor TextF where
  ...
  fmap f (LiftIO io k)               = LiftIO io (fmap f k)

renderTextSpan       :: MonadFree TextF m => TextSpan -> m ()
cursorUp             :: MonadFree TextF m => Int -> m ()
clearFromCursorToEnd :: MonadFree TextF m => m ()
clearLine            :: MonadFree TextF m => m ()
...
cursorDown :: MonadFree TextF m => Int -> m ()
cursorDown n = cursorUp (-n)

renderTextBlock :: MonadFree TextF m => TextBlock a -> m ()
...
renderTextBlocks :: MonadFree TextF m => TextBlocks -> m ()
renderTextBlocks = traverse_ renderTextBlock . getTextBlocks

reRenderTextBlocks :: MonadFree TextF m => TextBlocks -> TextBlocks -> m ()
...
textAlg :: MonadIO f => TextEnvironment f -> TextF (f b) -> f b
...
textAlg _                   (LiftIO io            k) = liftIO io >>= k

instance MonadIO (Free TextF) where
